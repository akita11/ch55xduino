...
volatile __xdata uint8_t UpPoint2_Busy =
...
void setControlLineStateHandler() {
  ...
  /*if ( ((controlLineState & 0x01) == 0) && (*((__xdata uint32_t *)LineCoding)
  == 1200) ){ //both linecoding and sdcc are little-endian pTaskFn tasksArr[1];
      USB_CTRL = 0;
      EA = 0; //Disabling all interrupts is required. tasksArr[0] =
...
uint8_t USBSerial_write(char c) { // 3 bytes generic pointer
  ...
  if (controlLineState > 0) {
    while (true) {
      waitWriteCount = 0;
      while (UpPoint2_Busy) { // wait for 250ms or give up, on my mac it takes
                              // about 256us
        waitWriteCount++;
        delayMicroseconds(5);
